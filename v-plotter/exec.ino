...
boolean exec_executeBasicCommand(String inCmd, String inParam1, String inParam2, String inParam3, String inParam4, int inNoOfParams)
{
  ...
  }else if (inCmd.startsWith(CMD_SETMACHINENAME)){
...
{
  Serial.println(F(" CAMBIO DE NOMBRE DE MAQUINA NO IMPLEMENTADO (exec_setMachineNameFromCommand) "));
}
...
void exec_changePenWidth()
{
  ...
  Serial.println(F("    envio pen width "));
...
void exec_drawBetweenPoints(float p1a, float p1b, float p2a, float p2b, int maxSegmentLength)
{
  ...
  if (c2x > 20 && c2x < machineSizeSteps.x - 20 && c2y > 20 && c2y < machineSizeSteps.y - 20 && c1x > 20 && c1x < machineSizeSteps.x - 20 && c1y > 20 && c1y < machineSizeSteps.y - 20){
    ...
    //agrego estas dos lineas para que apague los motores despues de un rato
      powerIsOn = true;
